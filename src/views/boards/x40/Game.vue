        ...
        <v-layout>
            <v-flex align-center xs12>
                <svg viewBox="0 0 400 400">
                    ...
                    <template v-if="match" v-for="(piece, key) in pieces">
                        <transition name="fade" :key="`piece-${key}`">
                            <circle 
                                v-if="piece.position"
                                @click="handleClick({letter: piece.position})"
                                @mouseover="mouseover = piece.position"
...
export default {
    ...
    methods: {
        ...
        rematch () {
            ...
            this.$router.push(`/x40/${match.matchId}`)
